...
const ExpenseForm = (props) => {
  ...
  useEffect(() => {
    fetch(
      `https://movies-2bfe4-default-rtdb.firebaseio.com/${cleanUserEmail}expenses.json`,
      {
        method: "GET",
        headers: {
  ...
  const editExpenseClickHandler = (event) => {
    ...
    fetch(
      `https://movies-2bfe4-default-rtdb.firebaseio.com/${cleanUserEmail}expenses/${event.target.id}.json`,
      {
        method: "DELETE",
      }
    ).then((res) => {
